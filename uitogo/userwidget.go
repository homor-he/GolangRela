...
func (this *MY_UIPerClient) SetupUI(PerClient *widgets.QFrame) {
	...
	this.Check.ConnectClicked(func(checked bool) {
		account := this.AccountInput.Text()
		pwd := this.PwdInput.Text()
		//tcpconn.SendGeneralLoginPkg()
		jsonStr := fmt.Sprintf("{\"User_ID\": \"%s\",\"Password\": \"%s\"}", account, pwd)

		userInfo := this.ItemLink.Connect.UserInfo
		if userInfo != nil {
			this.AddLog("账号:%d 用户名:%s 已登录，请先登出该账号", userInfo.UserID, userInfo.UserName)
		} else {
			err := tcpconn.SendGeneralLoginPkg(this.ItemLink.Connect, jsonStr)
			if err == nil {
				userNewInfo := this.ItemLink.Connect.UserInfo
				if userNewInfo != nil {
					this.AddLog("账号:%d 用户名:%s 登录成功", userNewInfo.UserID, userNewInfo.UserName)
				} else {
					this.AddLog("账号:%s 登录失败", account)
				}

			} else {
				//this.AddLog("账号:%d 用户名:%s 登录成功", userInfo.UserID, userInfo.UserName)
				this.AddLog("账号:%s 登录失败", account)
			}
		}
	})
	...
	this.Logout.ConnectClicked(func(checked bool) {
		userInfo := this.ItemLink.Connect.UserInfo
		if userInfo != nil {
			uid := userInfo.UserID
			userName := userInfo.UserName
			err := tcpconn.SendLogout(this.ItemLink.Connect, userInfo.UserID)
			if err == nil {
				this.AddLog("账号:%d 用户名:%s 登出成功", uid, userName)
			}
		}
	})
...
func (this *MY_UIPerClient) RetranslateUi(PerClient *widgets.QFrame) {
...
func (this *MY_UIPerClient) Show() {
	this.Frame.Show()
}

func (this *MY_UIPerClient) SetLink(item *ItemInfo) {
	this.ItemLink = item
}

func (this *MY_UIPerClient) AddLog(format string, v ...any) {
